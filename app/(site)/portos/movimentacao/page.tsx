...
function corPorHhi(hhi: number) {
  if (hhi > 2500) return '#A0153E'; // 🔴 crítico — agir
  if (hhi > 1500) return '#D4922A'; // 🟡 monitorar
  return '#00a652';                 // 🟢 saudável
}

function nivelHhi(hhi: number) {
  ...
  return (
    <main className="max-w-screen-xl mx-auto px-4 md:px-6 py-10 space-y-8">
      ...
      <div className="flex items-start gap-3 bg-azul-medio/50 border border-white/5 rounded-xl p-4 text-xs text-gray-500">
        ...
        <div className="space-y-1">
          <p>
            Os dados disponibilizados neste painel são atualizados de forma recorrente a partir de fontes públicas de informação e podem sofrer alterações a qualquer momento, conforme as atualizações publicadas pelos canais oficiais e pelas as verificações técnicas realizadas pela equipe do Observatório de dados do IBI.
...
function ConcentracaoChart({ data, mes }: { data: ConcentracaoCarga[]; mes: string }) {
  ...
  // ── E1.1 — KPIs de concentração (dinâmicos, sem 'todos') ──────────────────
  const porNatureza = data.filter(d => d.naturezaKey !== 'todos');
  const maisConc = porNatureza[0];                       // já vem ordenado por CR4 desc
  const menosConc = porNatureza[porNatureza.length - 1];
  const agregada = data.find(d => d.naturezaKey === 'todos');

  // string condicional do card do agregado — compara CR4 do agregado com os extremos
  const textoAgregada = agregada && menosConc && maisConc
    ? agregada.cr4 < menosConc.cr4
      ? 'menos concentrado que qualquer carga isolada'
      : agregada.cr4 < maisConc.cr4
        ? 'concentração intermediária entre as cargas'
        : 'mais concentrado que as cargas individuais'
    : '';
  ...
  return (
    <div className="w-full">
      ...
      {/* Cards de KPI — concentração, cor dinâmica por nível de risco (HHI) */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        <KpiCard
          label="Carga mais concentrada"
          value={maisConc ? fmtPct(maisConc.cr4, false) : '—'}
          sub={maisConc
            ? `${maisConc.natureza} · HHI ${Math.round(maisConc.hhi).toLocaleString('pt-BR')} (${nivelHhi(maisConc.hhi)})`
            : ''}
          color={maisConc ? corPorHhi(maisConc.hhi) : '#0099d8'}
        />
        <KpiCard
          label="Carga mais pulverizada"
          value={menosConc ? fmtPct(menosConc.cr4, false) : '—'}
          sub={menosConc
            ? `${menosConc.natureza} · HHI ${Math.round(menosConc.hhi).toLocaleString('pt-BR')} (${nivelHhi(menosConc.hhi)})`
            : ''}
          color={menosConc ? corPorHhi(menosConc.hhi) : '#00a652'}
        />
        <KpiCard
          label="Agregado nacional (todas as cargas)"
          value={agregada ? fmtPct(agregada.cr4, false) : '—'}
          sub={agregada ? `HHI ${Math.round(agregada.hhi).toLocaleString('pt-BR')} · ${textoAgregada}` : ''}
          color="#0099d8"
        />
      </div>
