...
export function projetaCruzamentoCalado(
  ...
  for (const pt of recessao.pontos) {
    ...
    const cmrAlta    = cmrDeItacoatiara(pt.cota_ic80_max);
    const cmrBaixa   = cmrDeItacoatiara(pt.cota_ic80_min);

    if (centralDate === null && cmrCentral < calado_alvo) centralDate = pt.data;
    if (otimDate    === null && cmrAlta    < calado_alvo) otimDate    = pt.data;
    if (pessimDate  === null && cmrBaixa   < calado_alvo) pessimDate  = pt.data;
  }
  ...
  return {
    ...
    data_otimista:      otimDate,
    data_pessimista:    pessimDate,
    dias_central:       dias(centralDate),
    dias_otimista:      dias(otimDate),
    dias_pessimista:    dias(pessimDate),
